...
for i in A B C D; do
  ...
  ffmpeg -y -f lavfi -i "testsrc=size=1280x720:rate=30" \
    ...
    -t $DURATION -c:v libx264 -preset ultrafast -g 150 -sc_threshold 0 -b:v 1500k \
    -f hls -hls_time 5 -hls_list_size 0 -hls_flags split_by_time -hls_segment_filename "$dir/seg_%03d.ts" \
    "$dir/playlist.m3u8"
done
...
mkdir -p "$dir"
# Use the MP3 file and trim to exact duration
# -t comes AFTER -i to trim during encoding (more accurate)
# atrim filter ensures exact audio duration
ffmpeg -y -i "$BASE_AUDIO_FILE" \
  -af "atrim=duration=$DURATION,asetpts=PTS-STARTPTS" \
  -t $DURATION -c:a aac -ar 48000 -ac 2 -b:a 128k \
  -hls_time 5 -hls_list_size 0 -hls_flags split_by_time \
  -hls_segment_filename "$dir/seg_%03d.aac" \
...
#EXT-X-STREAM-INF:BANDWIDTH=2000001,RESOLUTION=1280x720,CODECS="avc1.42e01e,mp4a.40.2",AUDIO="audio",NAME="Stream A"
videoA/playlist.m3u8

# --- Video Variant 2 (Stream B) ---
#EXT-X-STREAM-INF:BANDWIDTH=2000002,RESOLUTION=1280x720,CODECS="avc1.42e01e,mp4a.40.2",AUDIO="audio",NAME="Stream B"
videoB/playlist.m3u8

# --- Video Variant 3 (Stream C) ---
#EXT-X-STREAM-INF:BANDWIDTH=2000003,RESOLUTION=1280x720,CODECS="avc1.42e01e,mp4a.40.2",AUDIO="audio",NAME="Stream C"
videoC/playlist.m3u8

# --- Video Variant 4 (Stream D) ---
#EXT-X-STREAM-INF:BANDWIDTH=2000004,RESOLUTION=1280x720,CODECS="avc1.42e01e,mp4a.40.2",AUDIO="audio",NAME="Stream D"
